...
const segmentToString = (segment) =>
  "[" +
  pointToString(startSegment(segment)) +
  ", " +
  pointToString(endSegment(segment)) +
  "]";
const midpointSegment = (segment) =>
  cons(
    (car(startSegment(segment)) + car(endSegment(segment))) / 2,
    (cdr(startSegment(segment)) + cdr(endSegment(segment))) / 2
  );
